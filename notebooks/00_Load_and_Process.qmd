...
- **Methods**: InteriorPoint, Manifold, QSA
- **Configurations**: Different parameter settings for each method
- **Graph Types**: BA, ER, LRM_ER_rewired
...
DATA_DIR = Path("../data/pidnebesna")

# Directory names
METHOD_DIRS = ["InteriorPoint", "Manifold", "QSA"]
GRAPH_TYPES = ["BA", "ER", "LRM_ER_rewired"]
...
INSTANCE_PATTERN = re.compile(
    ...
    r"(?:_rew(?P<rew>\d+))?_sim(?P<sim_id>\d+)"
)
```
...
def load_graph_instance(
    ...
    Args:
        ...
        graph_type: Type of graph (BA, ER, LRM_ER_rewired)
    ...
    if graph_type == "LRM_ER":
        ...
        base_name = f"LRM_ER_nNodes{n_nodes}_density{density}_rew{rew}"
    else:
        graph_type_dir = graph_type
        base_name = f"{graph_type}_nNodes{n_nodes}_density{density}"

    pickle_path = data_dir / graph_type_dir / f"{base_name}.p"

    if not pickle_path.exists():
        raise FileNotFoundError(f"Graph instance file not found: {pickle_path}")
...
df.to_pickle(output_dir / "graph_symmetry_results.pkl")
print(f"Saved pickle file: {output_dir / 'graph_symmetry_results.pkl'}")

# Save as CSV (note: permutation column will be string representation)
df.to_csv(output_dir / "graph_symmetry_results.csv", index=False)
print(f"Saved CSV file: {output_dir / 'graph_symmetry_results.csv'}")
