...
- Random Geometric Graphs (GEO) in 2D Euclidean space modeling spatial networks
...
graph_types = ["ER", "BA", "LRM", "GEO", "GEO3D", "DD", "HK"]
...
# Standard graphs: ER, BA, GEO, GEO3D, DD, HK (no rewiring parameter)
standard_graph_types = ["ER", "BA", "GEO", "GEO3D", "DD", "HK"]
...
# Plot for standard graph types (ER, BA, GEO, GEO3D, DD, HK)
standard_graph_types = ["ER", "BA", "GEO", "GEO3D", "DD", "HK"]
for graph_type in standard_graph_types:
...
for method in ["DR", "OR"]:
    for density in sorted(temp_best_basic['density'].unique()):
        ...
        fig = plt.figure(figsize=(5*n_cols, 3.5*n_rows), constrained_layout=True)
        gs = fig.add_gridspec(n_rows, n_cols, hspace=0.15, wspace=0.15)
...
for graph_type in standard_graph_types:
    ...
    g = sns.catplot(
        ...
        y='symmetry',
        hue='method_display',
        palette={METHOD_NAMES[m]: METHOD_COLORS[m] for m in ["SoftSort", "OT4P4AS"]},
        col='density',
        kind='box',
        showfliers=False,
        height=4,
...
#| fig-cap: "Heatmap visualization of statistical comparisons between convex and indefinite loss functions for standard graph types (ER, BA, GEO, GEO3D, DD, HK). Each panel shows p-values (left) and Cohen's D effect sizes (right) for different combinations of parameters. Rows represent different densities (ρ), columns represent different temperatures (τ). Within each cell, the heatmap shows penalty parameter (c) on y-axis and network size (n) on x-axis. Darker colors in p-value heatmaps indicate stronger statistical significance; in Cohen's D heatmaps, more negative values (blue) indicate convex superiority."
#| fig-width: 18
#| fig-height: 12
...
for method in ["SoftSort", "OT4P4AS"]:
    ...
    if not method_data.empty:
        ...
        standard_types = ["ER", "BA", "GEO", "GEO3D", "DD", "HK"]
...
#| code-summary: "Prepare data for unified comparison using optimal configurations from ranked voting analysis"

# Extract optimal values from previous analyses for reference
...
or_optimal_c = 0.1#optimal_temp_df[optimal_temp_df['Method'] == 'OR']['Optimal c'].iloc[0]

# Define optimal configurations for each method
# Values loaded from ranked voting analyses (can be manually overridden if needed)
optimal_configs = {
    'QSA': {
        'c': qsa_optimal_c
    },
...
print("Optimal configurations loaded from ranked voting analysis:")
...
```{python}
#| label: tbl-optimal-parameters-combined
#| tbl-cap: "Combined optimal parameters for all methods selected using ranked voting across graph topologies."

# Combine optimal parameters from iterative and temperature methods
combined_params = []

# Add iterative methods (QSA, MF, IP)
for _, row in optimal_c_df.iterrows():
    combined_params.append({
        'Method': row['Method'],
        'c': row['Optimal c'],
        'tau': None,
        'Loss': None,
        'Selection': row['Selection'] if 'Selection' in row else None,
        'Success Rate': row.get('Success Rate', None),
        'Mean Symmetry': row.get('Mean Symmetry', None)
    })
...
for _, row in optimal_temp_df.iterrows():
    combined_params.append({
        ...
        'c': row['Optimal c'],
        'tau': row['Optimal tau'],
        'Loss': 'convex',  # Both use convex loss in optimal config
        'Selection': row['Selection'],
        'Success Rate': row.get('Success Rate', None),
        'Mean Symmetry': row.get('Mean Symmetry', None)
    })

combined_params_df = pd.DataFrame(combined_params)

# Reorder columns for clarity
column_order = ['Method', 'c', 'tau', 'Loss', 'Selection', 'Success Rate', 'Mean Symmetry']
combined_params_df = combined_params_df[column_order]

print(combined_params_df.to_markdown(index=False, floatfmt=".4f"))

# Save combined table
save_table(
    combined_params_df,
    "optimal_parameters_combined",
    "Combined optimal parameters for all methods (ranked voting)",
    label="tab:optimal-parameters-combined"
)
```
...
standard_graph_types = [gt for gt in ['ER', 'BA', 'GEO', 'GEO3D', 'DD', 'HK'] if gt in unified_best['graph_type'].values]
...
def create_cd_diagram(ax, data, title):
    ...
    method_display_colors = {
        ...
        'OR': METHOD_COLORS['OT4P4AS'],
        'DR': METHOD_COLORS['SoftSort']
    }
...
#| fig-cap: "P-value heatmaps from pairwise t-tests stratified by network size and density for standard graph types (ER, BA, GEO, GEO3D, DD, HK). Each row represents a different network size (n), each column represents a different density (ρ). Darker colors and asterisks indicate stronger statistical significance."
#| fig-width: 16
#| fig-height: 14
...
#| fig-cap: "Cohen's D effect size heatmaps stratified by network size and density for standard graph types (ER, BA, GEO, GEO3D, DD, HK). Each row represents a different network size (n), each column represents a different density (ρ). Blue indicates row method outperforms column method."
#| fig-width: 16
#| fig-height: 14
...
#| fig-cap: "Critical difference diagrams stratified by network size and density for standard graph types (ER, BA, GEO, GEO3D, DD, HK). Each subplot shows the performance ranking for a specific network size and density combination. Groups connected by horizontal bars are not significantly different."
#| fig-width: 16
#| fig-height: 14
...
def create_composite_figure(
    ...
    gs_right_top = gs_right[0].subgridspec(1, 2)
    ...
    # Create the heatmap with seaborn (which handles colorbar positioning well)
    ...
    heatmap_pval = sns.heatmap(
        ...
        cbar_kws={
            'label': 'Significance',
            'ticks': [0, 1, 2, 3],  # Only show non-diagonal values
            'boundaries': [-0.5, 0.5, 1.5, 2.5, 3.5]  # Boundaries between discrete colors
        },
        ax=ax_pval,
        linewidths=0.5,
        linecolor='0',
    ...
    cbar = heatmap_pval.collections[0].colorbar
...
standard_graph_types = ['BA', 'ER', 'GEO', 'GEO3D', 'DD', 'HK']
